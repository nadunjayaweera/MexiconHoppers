...
export default function ProductList() {
  ...
  React.useEffect(() => {
    const fetchSalesData = async () => {
      try {
        const response = await axios.get("http://acdassanayake.me:8080/api/v1/getsales");
  ...
  const columns = [
    ...
    { field: "orderid", headerName: "Order ID", type: "number", width: 120, },
    {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <IconButton
          color="primary"
          size="small"
          onClick={() => handleUpdateStatus(params.row.feeldid, params.row.orderstatus)}
        >
          Update
        </IconButton>
      ),
 ...
 const handleUpdateStatus = async (feeldid) => {
  try {
    ...
    await axios.put(`http://acdassanayake.me:8080/api/v1/${feeldid}/update-data`, {});
  ...
  return (
    <div style={{ height: "100%", width: "100%" }}>
      ...
      />
      <OrderDetailsDialog open={openDialog} onClose={handleCloseDialog} orders={selectedOrder}/>
      <OrdersDetails open={openDialog2} onClose={handleCloseDialog2} orders={selectedOrder}/>
    </div>
  );
}
