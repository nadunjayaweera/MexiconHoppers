...
export default function ProductList() {
  ...
  React.useEffect(() => {
    const fetchSalesData = async () => {
      try {
        const response = await axios.get('http://acdassanayake.me:8080/api/v1/data/items');
  ...
  navigate(`/products/edit/${product.pid}`, { state: { product: product } });
};
  ...
  const handleDelete = (pid) => {
    ...
    if (confirmed) {
      fetch(`http://acdassanayake.me:8080/api/v1/deleteitem/${pid}`, {
        method: 'DELETE',
        headers: {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <>
          ...
          <IconButton aria-label="delete" color="error" onClick={() => handleDelete(params.row.pid)}>
            <DeleteIcon />
          </IconButton>
        </>
      ),
